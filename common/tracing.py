...
def setup_tracing(service_name, instrument_django=False):
    """
    Setup OpenTelemetry tracing for a service.

    Args:
        service_name: Name of the service for tracing identification
        instrument_django: Whether to instrument Django (should be False during initial setup,
                          and called separately after Django initialization)
    """
    ...
    # Only instrument Django if explicitly requested (after Django is initialized)
...
def instrument_django_tracing():
    """
    Instrument Django after it has been initialized.
    This should be called from Django's AppConfig.ready() method.
    """
    try:
        from opentelemetry.instrumentation.django import DjangoInstrumentor
